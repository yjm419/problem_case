...
public class TestYjm {
    ...
    public static void main(String[] args) throws ParseException {
        ...
        yMap.forEach((key,value)-> {
            ...
            rebarJsonMap.put("consumeWeightObjList", Arrays.toString(consumeWeightObjMap.keySet().stream().map(key1 -> "["+key1 + "," + consumeWeightObjMap.get(key1)+"]").toArray(String[]::new)));
            rebarJsonMap.put("inventoryUsableList", Arrays.toString(inventoryUsableMap.keySet().stream().map(key1 -> "["+key1 + "," + inventoryUsableMap.get(key1)+"]").toArray(String[]::new)));
            rebarJsonMap.put("arrivalWeightList", Arrays.toString(arrivalWeightMap.keySet().stream().map(key1 -> arrivalWeightMap.get(key1)).toArray(Double[]::new)));
            rebarJsonMap.put("testWeightList", Arrays.toString(testWeightMap.keySet().stream().map(key1 -> testWeightMap.get(key1)).toArray(Double[]::new)));
            rebarJsonMap.put("dateList", Arrays.toString(dateMap.keySet().stream().map(key1 -> dateMap.get(key1)).toArray(String[]::new)));
